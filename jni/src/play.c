...
//static int dst_fix_fmt = PIX_FMT_RGBA;
//static int display_fix_fmt = SDL_PIXELFORMAT_RGBA8888;
...
static int MIN_FRAMES = 120;
static int max_analyze_duration = 0;
...
/* return < 0 if aborted, 0 if no packet and > 0 if delete last frames successfully.  */
static int packet_queue_delete(PacketQueue *q, int block) {
	AVPacketList *pkt1;
	int ret;
	...
	for (;;) {
		...
		if (pkt1) {
			...
			if(tmp == NULL){
				...
				q->last_pkt = second;
				q->nb_packets--;
				q->size -= tmp->pkt.size + sizeof(*tmp);
			    av_free(tmp);
			    ret = 1;
			    break;
			}	
...
static void video_image_display(VideoState *is) {
	...
	if (vp->pFrameRGB) {
		...
		if (aspect_ratio <= 0.0)
			aspect_ratio = 1.0;
		if(g_aspect_ratio_type == 0){
		    // xiao ping original size
		    aspect_ratio *= (float) vp->width / (float) vp->height;
		} else if(g_aspect_ratio_type == 1) {
		   // xiao ping full screen
		   aspect_ratio = (screen_width * 1.0) / screen_height;
		} else if(g_aspect_ratio_type == 2) {
		    // da ping  4:3
		    aspect_ratio = (4 * 1.0)/ 3;
		} else if(g_aspect_ratio_type == 3) {
		    // da ping full screen la seng
		...
		if(g_aspect_ratio_type == 0){
		   // xiao pin original size
		    ...
		    rect.y = fs_screen_height - height - (screen_height - height) / 2;
		    rect.w = FFMAX(width, 1);
		    rect.h = FFMAX(height, 1);
		} else if(g_aspect_ratio_type == 1) {
		    // xiao ping full screen
			rect.x = 0;
		    rect.y = fs_screen_height - screen_height ;
		    rect.w = screen_width;
		    rect.h = screen_height;
		    
		} else if(g_aspect_ratio_type == 2) {
		    // da ping 4:3
		    rect.h = FFMAX(fs_screen_height, 1);
			rect.w = (int)(rect.h * aspect_ratio);
			rect.x = (fs_screen_width - rect.w) / 2;
            rect.y = 0;
		} else if(g_aspect_ratio_type == 3){
		     // da ping full screen
		    rect.x = 0;
            rect.y = 0;
		    rect.w = FFMAX(fs_screen_width, 1);
		    rect.h = FFMAX(fs_screen_height, 1);
		}
		...
		if (NULL == texture) {
			...
			if (!texture) {
				LOGV( "Couldn't set create texture: %s\n", SDL_GetError());
				exit(1);
			}
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts1,
	...
	LOGV("frame_type=%c clock=%0.3f pts=%0.3f\n",
	...
	if (!vp->pFrameRGB ||
#endif
			vp->reallocate || vp->width != src_frame->width
	...
	if (vp->pFrameRGB) {
		...
		if (is->img_convert_ctx == NULL) {
			LOGV( "Cannot initialize the conversion context\n");
			exit(1);
		}
...
static int get_video_frame(VideoState *is, AVFrame *frame, int64_t *pts,
    ...
    if(delay > SKIP_DELAY_TIME && g_skip){
          ...
          LOGV("Skip delay :%.3f, video: %.3f, mask clock:%.3f\n", delay, get_video_clock(is),get_master_clock(is));
      }else
...
//		double vclock = get_video_clock( is );
//		double aclock = get_audio_clock( is );
//		double sync_diff = vclock - aclock;
//		if ( sync_diff >= 0.04 && sync_diff < 4) {
//			usleep( sync_diff * 1000000 );
//		}
        	
		pos = pkt.pos;
		av_free_packet(&pkt);
		
//		if(ret == 0 && ((frame->width != 0 && frame->width != is->video_st->codec->width) || (frame->height != 0 && frame->height != is->video_st->codec->height))) 
//		goto the_end;
        ...
        if(ori_width == 0 || ori_width != frame->width || ori_width != is->video_st->codec->width || 
		   ori_height == 0 || ori_height != frame->height ||  ori_height !=  is->video_st->codec->height)
		    continue;		
...
#ifdef DEBUG
		LOGV("video_st time_base: %f, pts_int:%d\n",
				av_q2d(is->video_st->time_base), pts_int);
#endif
        // add by lb
        AVCodecContext *h = frame->owner;
		if(h->flags2 ==4){
		    // remove 5 frames from video queue
//			int i = 0;
//			AVPacket pkt1;
//			for ( i = 0; i < 5; i ++){
//			    packet_queue_delete(&is->videoq, 1);
//			}
			
		    count = 50;
			LOGV("masaic found, skip");
		}
		// add by lb end
		if(count ==0){
		   ...
		   LOGV("qrop_picture");
		    if(frame->key_frame == 1 && h->flags2 !=4) {
			    ret = queue_picture(is, frame, pts, pos);
		        count = 0; 
			} else {
			    count--;
			...
			if (is->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
				...
				if (fabs(avg_diff) >= is->audio_diff_threshold) {
					...
					max_nb_samples = ((nb_samples
							* (100 + SAMPLE_CORRECTION_PERCENT_MAX) / 100));
					wanted_nb_samples =
							FFMIN(FFMAX(wanted_nb_samples, min_nb_samples), max_nb_samples);
				}
				av_dlog(NULL,
						"diff=%f adiff=%f sample_diff=%d apts=%0.3f vpts=%0.3f %f\n", diff, avg_diff, wanted_nb_samples - nb_samples, is->audio_clock, is->video_clock, is->audio_diff_threshold);
			}
		} else {
		...
		while (pkt_temp->size > 0 || (!pkt_temp->data && new_packet)) {
			...
			if (dec->sample_fmt != is->audio_src_fmt
				...
				if (!is->swr_ctx || swr_init(is->swr_ctx) < 0) {
					LOGV(
							"Cannot create sample rate converter for conversion of %d Hz %s %d channels to %d Hz %s %d channels!\n", dec->sample_rate, av_get_sample_fmt_name(dec->sample_fmt), dec->channels, is->audio_tgt_freq, av_get_sample_fmt_name(is->audio_tgt_fmt), is->audio_tgt_channels);
					break;
				}
			...
			if (is->swr_ctx) {
				...
				if (wanted_nb_samples != is->frame->nb_samples) {
					if (swr_set_compensation(is->swr_ctx,
							...
							wanted_nb_samples * is->audio_tgt_freq
									/ dec->sample_rate) < 0) {
						LOGV( "swr_set_compensation() failed\n");
						break;
					}
				}
				...
				if (len2 < 0) {
					LOGV( "audio_resample() failed\n");
					break;
				}
				if (len2
						== sizeof(is->audio_buf2) / is->audio_tgt_channels
								/ av_get_bytes_per_sample(is->audio_tgt_fmt)) {
					LOGV( "warning: audio buffer is probably too small\n");
					swr_init(is->swr_ctx);
				}
			...
			*pts_ptr = pts;
...
#ifdef DEBUG
			{
				...
				LOGV("audio: delay=%0.3f clock=%0.3f pts=%0.3f\n",
...
static int stream_component_open(VideoState *is, int stream_index) {
	...
	if (avctx->codec_type == AVMEDIA_TYPE_AUDIO) {
		...
		if (wanted_spec.freq <= 0 || wanted_spec.channels <= 0) {
			LOGV( "Invalid sample rate or channel count!\n");
			return -1;
		}
	}
...
//            break;
		...
		case KU_PLAYER_BUFCHK:
			//printf("yyyyyyyyyyyyyyyyyyyyy, %d\n", cur_stream->videoq.nb_packets);
		    if (cur_stream->load) {
		        LOGV("Loading..., packet:%d", cur_stream->videoq.nb_packets);
		        if( check_buffer(cur_stream, MIN_FRAMES)){
	#ifdef ANDORID
		        	Android_Notify(MSG_LOAD_FINISHED);
					LOGV("loading finished");
	#else
...
int player_init(){
...
#endif
	if (SDL_Init(flags)) {
		LOGV( "Could not initialize SDL - %s\n", SDL_GetError());
		LOGV( "(Did you set the DISPLAY variable?)\n");
	...
	if (av_lockmgr_register(lockmgr)) {
		LOGV( "Could not initialize lock manager!\n");
		return RET_INIT_ERROR;
	}
...
int player_prepare(const char *url, int minFrames, int maxAnalyzeDuration){
    setMaxAnalyzeDuration(maxAnalyzeDuration);
...
#else
	player_init();
	input_filename = "/home/xdpan/media/test.mp4";
	player_prepare(input_filename);
	player_main();
	player_exit();
	return 0;
#endif
...
void setMaxAnalyzeDuration(int a){
    max_analyze_duration = a;
}

void setMinFrames(int b){
      MIN_FRAMES = b;
}

int changeAudio(int index){
    LOGV("change Audio");
    // at first close exist audio
    if(g_stream == NULL) return -1;
	if (g_stream->audio_stream >= 0){
	    LOGV("previous stream index is %d", g_stream->audio_stream);
		stream_component_close(g_stream, g_stream->audio_stream);
	}
	
    // open a new audio
	if (index >= 0) {
		stream_component_open(g_stream, index);
	}
	return 0;
}
 ...
 //add by lb
char* getyingguiData(){
    char yingguidata[1000]="audioinfo:";
    if(g_stream == NULL)return	"none";
	AVFormatContext *ic = g_stream->ic;
	int i = 0;
//	AVDictionaryEntry *lang = av_dict_get(ic->streams[g_stream->last_audio_stream]->metadata, "language", NULL, 0);
//	if(lang && lang->value){
//	    strcat(yingguidata,lang->value);
//		strcat(yingguidata,"@");
//		char str1[4];
//       sprintf(str1,"%d",g_stream->last_audio_stream);
//		strcat(yingguidata,str1);
//		strcat(yingguidata,",");
//	}
	char buf[256];
	 avcodec_string(buf, sizeof(buf), ic->streams[g_stream->last_audio_stream]->codec, 1);
	strcat(yingguidata,buf);
	strcat(yingguidata,"@");
	char str1[4];
    sprintf(str1,"%d",g_stream->last_audio_stream);
    strcat(yingguidata,str1);
	...
	for(i = 0; i < ic->nb_streams; ++i){
	    char buf[256];
		if(ic->streams[i] == NULL) continue;
		AVCodecContext *avctx;
        avctx = ic->streams[i]->codec;
		if(avctx->codec_type == AVMEDIA_TYPE_AUDIO){
		    avcodec_string(buf, sizeof(buf), ic->streams[i]->codec, 1);
			LOGV(" %s: is format yingui", buf);
...
{
    g_aspect_ratio_type = aspect_ratio_type;
	return 0;
}
