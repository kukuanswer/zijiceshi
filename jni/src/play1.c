...
static int stream_component_open(VideoState *is, int stream_index) {
	...
	case AVMEDIA_TYPE_AUDIO:
        ...
        // add by lb
		char buf[256];
		avcodec_string(buf, sizeof(buf), is->audio_st->codec, 1);
		AVDictionaryEntry *lang = av_dict_get(is->audio_st->metadata, "language", NULL, 0);
			
	    LOGV("%s::: %s: is format yingui", buf, lang->value);
